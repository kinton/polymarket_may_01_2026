...
# Test risk limits (capital percentage per trade)
@pytest.mark.asyncio
async def test_check_risk_limits_sufficient_balance(mock_trader, cleanup_daily_limits):
    """Test that risk check passes when trade size is within limits."""
    # Mock sufficient balance ($1000, max 5% = $50)
    mock_trader.client.get_balance_allowance = MagicMock(
        return_value={"balance": int(1000 * 1e6), "allowances": {}}
    )

    mock_trader._planned_trade_amount = 10.0  # $10 trade
...
@pytest.mark.asyncio
async def test_check_risk_limits_exact_limit(mock_trader, cleanup_daily_limits):
    """Test that risk check passes when trade size equals 5% limit."""
    # Mock balance ($100, max 5% = $5)
    mock_trader.client.get_balance_allowance = MagicMock(
        return_value={"balance": int(100 * 1e6), "allowances": {}}
    )

    mock_trader._planned_trade_amount = 5.0  # Exactly at limit
...
@pytest.mark.asyncio
async def test_check_risk_limits_no_client(mock_trader, cleanup_daily_limits):
    """Test that risk check fails when CLOB client is not initialized."""
...
def test_track_daily_pnl_creates_file(mock_trader, cleanup_daily_limits):
    """Test that tracking daily PnL creates the JSON file."""

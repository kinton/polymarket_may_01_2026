...
def test_orderbook():
    ...
    try:
        ...
        print(f"\nOrderbook data:")
        print(f"  Market: {orderbook.market if hasattr(orderbook, 'market') else 'N/A'}")
        print(f"  Asset ID: {orderbook.asset_id if hasattr(orderbook, 'asset_id') else 'N/A'}")
        print(f"  Timestamp: {orderbook.timestamp if hasattr(orderbook, 'timestamp') else 'N/A'}")
        ...
        if asks and len(asks) > 0:
            ...
            if best_ask > 0.50:
                print(f"  → YES is winning (price > 0.50)")
            else:
                print(f"  → NO is winning (price <= 0.50)")

...
class TradingBotRunner:
    ...
    def __init__(self, dry_run: bool = True, trade_size: float = 1.0, poll_interval: int = 90, run_once: bool = False):
        ...
        self.finder_logger.info("="*80)
        self.finder_logger.info(f"Mode: {'DRY RUN (Safe Mode)' if self.dry_run else '🔴 LIVE TRADING 🔴'}")
        self.finder_logger.info(f"Trade Size: ${self.trade_size}")
        self.finder_logger.info(f"Poll Interval: {self.poll_interval}s")
        self.finder_logger.info(f"Trader Start Buffer: {self.TRADER_START_BUFFER}s before market close")
        self.finder_logger.info("="*80)
    ...
    async def start_trader_for_market(self, market: Dict[str, Any]):
        ...
        try:
            ...
            self.trader_logger.error(
                f"Error running trader for market {condition_id}: {e}",
                exc_info=True
            )
        finally:
            # Clean up
    ...
    async def poll_and_trade(self):
        ...
        while True:
            try:
                self.finder_logger.info(f"Polling for active markets... (every {self.poll_interval}s)")
...
async def main():
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Trade size in dollars (default: 1)"
    )
    parser.add_argument(
        "--poll-interval",
        type=int,

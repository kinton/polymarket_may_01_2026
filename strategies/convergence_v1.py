"""
Convergence Trading Strategy v1 for Polymarket "Up or Down" markets.

Core idea (May strategy):
  When |current_price - target| < 5bp, the outcome is ~50/50.
  If the market shows a strong skew (cheap side <= 35c), buy the cheap side.
...
class ConvergenceV1(BaseStrategy):
    ...
    Two phases:
    1. observe() -- called every tick during observation window (200s->20s)
    2. decide() -- evaluates accumulated data, returns signal or None
    """
    ...
    def __init__(
        ...
        min_cheap_price: float = 0.10,     # filter out <10c entries that skew data
    ...
    def observe(self, tick: MarketTick) -> None:
        ...
        oracle_snapshot = tick.oracle_snapshot
    ...
    def decide(self, tick: MarketTick) -> Signal | None:
        ...
        if not self.should_decide(tick.time_remaining):
            return None
        ...
        confidence = convergence_rate * side_consistency
